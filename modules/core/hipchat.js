 ...
 */
module.exports =  function hipchatBot( userConfig, channels, listenToMessages, displayDebugInfo, context, hipchatConfig )
{
    /**
     * ## sendResponse
     *
    ...
    function say( from, text, confObj )
    {
        ...
        if ( confObj && confObj.data && confObj.data.item && confObj.data.item.room &&
                confObj.data.item.room.links && confObj.data.item.room.links.self )
        {
    ...
    function serverFunction( request, response )
    {
        ...
        if ( request.method === 'POST' )
        {
          ...
          request.on( 'end', () =>
          {
            ...
            const message = messageObj.message.slice(1)
        ...
        {
            const botText = '{"status":"403","err":"invalid access (sorry)","text":""}';
