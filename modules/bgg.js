...
class Bgg extends Module {
  ...
  bgg(from, to, text, textArr) {
    return new Promise((resolve, reject) => {
      const { bggApiBaseUrl, req } = this.userConfig;

      const request = req.request;

      const cleanText = text.replace(/&amp;/g, '%26').replace(/ /g, '+');
      ...
      const callback = (error, response, body) => {
        if (body && !error) {
          ...
          if (!boardGame) {
            resolve(
              `Sorry ${to}, I didn't find anything. Maybe try a site search.\nwww.boardgamegeek.com/geeksearch.php?action=search&objecttype=boardgame&q=${cleanText}&B1=Go`
            );

            return null;
          } else if (Array.isArray(boardGame)) {
            boardGame = boardGame.sort(
              (a, b) =>
                parseInt(a.yearpublished._text) <
                parseInt(b.yearpublished._text)
            )[0];
            multipleHits = true;
          }
          ...
          const gameCb = (error, response, body) => {
            ...
            const mechanicArray = Array.isArray(mechanicRaw)
              ? mechanicRaw
              : [mechanicRaw];
            const categoryArray = Array.isArray(categoryRaw)
              ? categoryRaw
              : [categoryRaw];
            const pollsArray = Array.isArray(pollsRaw) ? pollsRaw : [pollsRaw];
            const familyArray = Array.isArray(familyRaw)
              ? familyRaw
              : [familyRaw];

            const name = namesArray.filter(
              n => n._attributes.primary === 'true'
            )[0]._text;
          ...
          resolve(
            'sorry.... something went wrong. maybe try again later or mention this to an admin'
          );
        }
      };
