...
class Nico extends Module {
  ...
  responses() {
    ...
    return {
      commands: {
        ...
        'whoisit?': {
          f: this.whosIt,
          desc: 'shows who is it',
          syntax: [`${trigger}whoisit?`],
        },

        'whosit?': {
          f: this.whosIt,
          desc: "shows who's it",
          syntax: [`${trigger}whosit?`],
        },
      },
    };
  }
  ...
  tag(from, to, text, textArr, command, confObj) {
    const { admins, nicoFlipped, nico } = this.userConfig;
